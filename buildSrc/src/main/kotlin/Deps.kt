...
object PluginDeps {
    ...
    val PluginMicronaut = DepPlugin("Micronaut", "io.micronaut.application", version = "3.4.0").also { it.add() }
...
object Deps {
    ...
    val H2               = Dep.from("com.h2database:h2:2.1.212").also { it.add(DB_GROUP) }
    ...
    val Postgresql       = Dep.from("org.postgresql:postgresql:42.3.5", versionRegex = "${THREEDIGITSs}\\.jre\\d*\$").also { it.add(DB_GROUP) }
    ...
    val KotlinxKaml      = Dep.from("com.charleskorn.kaml:kaml:0.44.0").also { it.add(SERIALIZATION_GROUP) }
    ...
    val Kotest = Dep.from("io.kotest:kotest-runner-junit5-jvm:5.3.0").also { it.add(TESTING_GROUP) }
