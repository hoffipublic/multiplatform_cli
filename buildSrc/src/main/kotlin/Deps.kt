...
object Deps {
    ...
    fun add(dep: Dep, key: Any): Dep { APPLIED_DEPS.getOrPut(key.javaClass.simpleName){setOf(dep).toMutableSet()} ; return dep }
    ...
    object Logging {
        ...
        val Logback = Dep.from("ch.qos.logback:logback-classic:1.2.10").also { VERSION_Logback = it.version ; add(it, Logging) }
    ...
    object Serialization {
        val KotlinxJson = Dep.from("org.jetbrains.kotlinx:kotlinx-serialization-json:1.3.2").also { add(it, Serialization) }
